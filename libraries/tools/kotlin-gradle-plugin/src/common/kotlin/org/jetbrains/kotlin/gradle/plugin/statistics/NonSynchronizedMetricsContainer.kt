...
class NonSynchronizedMetricsContainer : IStatisticsValuesConsumer, Serializable {
    ...
    override fun report(metric: BooleanMetrics, value: Boolean, subprojectName: String?, weight: Long?): Boolean {
        val metricContainer = booleanMetrics[MetricDescriptor(metric, subprojectName)] ?: metric.type.newMetricContainer()
            .also { booleanMetrics[MetricDescriptor(metric, subprojectName)] = it }
    ...
    override fun report(metric: NumericalMetrics, value: Long, subprojectName: String?, weight: Long?): Boolean {
        val metricContainer = numericalMetrics[MetricDescriptor(metric, subprojectName)] ?: metric.type.newMetricContainer()
            .also { numericalMetrics[MetricDescriptor(metric, subprojectName)] = it }
        metricContainer.addValue(value, weight)
        return true
    }

    override fun report(metric: StringMetrics, value: String, subprojectName: String?, weight: Long?): Boolean {
        val metricContainer = stringMetrics[MetricDescriptor(metric, subprojectName)] ?: metric.type.newMetricContainer()
            .also { stringMetrics[MetricDescriptor(metric, subprojectName)] = it }

        metricContainer.addValue(value, weight)
        return true
    }

    fun flush(metricConsumer: IStatisticsValuesConsumer) {
