...
class StringBuilderNativeTest {
    private fun assertEquals(actual: StringBuilder, expected: String) = assertEquals(expected, actual.toString())
    ...
    fun testInsertStringException(initial: String, index: Int, toInsert: String) {
        assertFailsWith<IndexOutOfBoundsException> { StringBuilder(initial).insert(index, toInsert) }
        assertFailsWith<IndexOutOfBoundsException> { StringBuilder(initial).insert(index, toInsert.toCharArray()) }
        assertFailsWith<IndexOutOfBoundsException> { StringBuilder(initial).insert(index, toInsert as CharSequence) }
    }
    ...
    fun testInsert() {
        ...
        // Incorrect indices.
        assertFailsWith<IndexOutOfBoundsException> { StringBuilder("a").insertRange(-1, "1", 0, 0) }
        assertFailsWith<IndexOutOfBoundsException> { StringBuilder("a").insertRange(2, "1", 0, 0) }
        assertFailsWith<IndexOutOfBoundsException> { StringBuilder("a").insertRange(1, "1", -1, 0) }
        assertFailsWith<IndexOutOfBoundsException> { StringBuilder("a").insertRange(1, "1", 0, 2) }
        assertFailsWith<IllegalArgumentException> { StringBuilder("a").insertRange(1, "123", 2, 0) }
