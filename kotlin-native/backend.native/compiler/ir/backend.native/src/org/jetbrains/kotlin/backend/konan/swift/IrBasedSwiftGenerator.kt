...
internal open class IrBasedSwiftVisitor : IrElementVisitor<Unit, IrBasedSwiftVisitor.Result> {
    ...
    override fun visitClass(declaration: IrClass, data: Result) {
        ...
        when(declaration.kind) {
            ...
            ClassKind.CLASS -> SwiftCode.build {
                val cls = `class`(
                        names.swift,
                        inheritedTypes = listOfNotNull(declaration.superClass?.let { Names(it).path.type } ?: kotlinAny.swift.type)
                ) {
    ...
    private fun mapTypeToC(declaration: IrType): CCode.Type? = CCode.build {
        when {
            ...
            declaration.isRegularClass || declaration.isInterface -> void.pointer(nullability = CCode.Type.Pointer.Nullability.NULLABLE.takeIf { declaration.isNullable() })
            else -> null
        }
    }
    ...
    private fun bridgeFor(declaration: IrType): Bridge? = SwiftCode.build {
        ...
        when {
            ...
            declaration.computeBinaryType() is BinaryType.Reference -> {
                ...
                val type = IrBasedSwiftVisitor.kotlinAny.swift.type.let { if (declaration.isNullable()) it.optional else it }
                return Bridge.Object(type, cType)
            }
            else -> return null
        }
